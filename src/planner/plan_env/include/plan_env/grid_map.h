...
#ifndef PLAN_ENV_PUBLIC
#if defined _WIN32 || defined __CYGWIN__
  #ifdef BUILDING_PLAN_ENV
    #define PLAN_ENV_PUBLIC __declspec(dllexport)
  #else
    #define PLAN_ENV_PUBLIC __declspec(dllimport)
  #endif
#else
  #define PLAN_ENV_PUBLIC __attribute__((visibility("default")))
#endif
#endif
...
class PLAN_ENV_PUBLIC GridMap {
public:
  PLAN_ENV_PUBLIC GridMap() {}
  PLAN_ENV_PUBLIC ~GridMap() {}
  ...
  PLAN_ENV_PUBLIC void initMap(std::shared_ptr<rclcpp::Node>& nh);
  ...
  int getVoxelNum();
  PLAN_ENV_PUBLIC bool getOdomDepthTimeout() { return md_.flag_depth_odom_timeout_; }
...
private:
  ...
  void extrinsicCallback(const nav_msgs::msg::Odometry::SharedPtr odom);
...
PLAN_ENV_PUBLIC inline int GridMap::toAddress(const Eigen::Vector3i& id) {
  return id(0) * mp_.map_voxel_num_(1) * mp_.map_voxel_num_(2) + id(1) * mp_.map_voxel_num_(2) + id(2);
}

PLAN_ENV_PUBLIC inline int GridMap::toAddress(int& x, int& y, int& z) {
  return x * mp_.map_voxel_num_(1) * mp_.map_voxel_num_(2) + y * mp_.map_voxel_num_(2) + z;
}

PLAN_ENV_PUBLIC inline void GridMap::boundIndex(Eigen::Vector3i& id) {
  Eigen::Vector3i id1;
...
PLAN_ENV_PUBLIC inline bool GridMap::isUnknown(const Eigen::Vector3i& id) {
  Eigen::Vector3i id1 = id;
  boundIndex(id1);
  return md_.occupancy_buffer_[toAddress(id1)] < mp_.clamp_min_log_ - 1e-3;
}

PLAN_ENV_PUBLIC inline bool GridMap::isUnknown(const Eigen::Vector3d& pos) {
  Eigen::Vector3i idc;
  posToIndex(pos, idc);
  return isUnknown(idc);
}

PLAN_ENV_PUBLIC inline bool GridMap::isKnownFree(const Eigen::Vector3i& id) {
  Eigen::Vector3i id1 = id;
  boundIndex(id1);
...
PLAN_ENV_PUBLIC inline bool GridMap::isKnownOccupied(const Eigen::Vector3i& id) {
  Eigen::Vector3i id1 = id;
  boundIndex(id1);
  int adr = toAddress(id1);
...
PLAN_ENV_PUBLIC inline void GridMap::setOccupied(Eigen::Vector3d pos) {
...
PLAN_ENV_PUBLIC inline void GridMap::setOccupancy(Eigen::Vector3d pos, double occ) {
  if (occ != 1 && occ != 0) {
...
PLAN_ENV_PUBLIC inline int GridMap::getOccupancy(Eigen::Vector3d pos) {
...
PLAN_ENV_PUBLIC inline int GridMap::getInflateOccupancy(Eigen::Vector3d pos) {
  if (!isInMap(pos)) return -1;
...
PLAN_ENV_PUBLIC inline int GridMap::getOccupancy(Eigen::Vector3i id) {
...
PLAN_ENV_PUBLIC inline bool GridMap::isInMap(const Eigen::Vector3d& pos) {
...
PLAN_ENV_PUBLIC inline bool GridMap::isInMap(const Eigen::Vector3i& idx) {
...
PLAN_ENV_PUBLIC inline void GridMap::posToIndex(const Eigen::Vector3d& pos, Eigen::Vector3i& id) {
  for (int i = 0; i < 3; ++i) id(i) = floor((pos(i) - mp_.map_origin_(i)) * mp_.resolution_inv_);
}

PLAN_ENV_PUBLIC inline void GridMap::indexToPos(const Eigen::Vector3i& id, Eigen::Vector3d& pos) {
  for (int i = 0; i < 3; ++i) pos(i) = (id(i) + 0.5) * mp_.resolution_ + mp_.map_origin_(i);
}

PLAN_ENV_PUBLIC inline void GridMap::inflatePoint(const Eigen::Vector3i& pt, int step, vector<Eigen::Vector3i>& pts) {
  int num = 0;
...
PLAN_ENV_PUBLIC inline double GridMap::getResolution() { return mp_.resolution_; }
